...
class BatchAgent(object):
    ...
    def batch_learn(self, experiences):
        """
        Sadly, we have to use the representation in terms of phi_sa r phi_ns
        :param experiences:
        :return: Nothing
        """
        # experience is in (phi_s, a, r, phi_ns)
        (phi_s, policy_s, actions, rewards, phi_ns, policy_ns, sample_indices) = experiences.get_batch_data()
        ...
        if spl_targets is not None:
            for s_idx, target in zip(self.domain.spl_indexs, spl_targets):
                y[s_idx] = target
        print "Begin batch fitting"
        self.behavior_representation.model.fit(x=phi_s, y=y, batch_size=experiences.mini_batch_size, nb_epoch=10, verbose=1)
